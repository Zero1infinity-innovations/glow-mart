        ...
        <form class="row g-3" id="productForm">
            ...
            <div class="col-md-4">
                ...
                <select name="unit" id="unit" class="form-control">
                    ...
                    <option value="Quantity" @selected($product->unit=="Quantity")>Quantity</option>
                    <option value="Gram" @selected($product->unit=="Gram")>Gram</option>
                    <option value="Kilogram" @selected($product->unit=="Kilogram")>Kilogram</option>
                    <option value="Milliliter" @selected($product->unit=="Milliliter")>Milliliter</option>
            ...
            <div class="col-md-4">
                ...
                <input type="hidden" name="old_image[]"  value="{{$product->multi_image}}">
                <input type="file" name="multi_image[]" id="multi_image" class="form-control" multiple />
                @if(count($product->multi_image)>0)
...
<script>
    productForm.onsubmit = async (e) =>{
        e.preventDefault();
        makePostRequest("{{route('admin.product.update')}}",productForm,'productForm')
    }
</script>
@endpush